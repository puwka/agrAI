...
const VOICE_PREVIEWS_ROOT = path.join(process.cwd(), "public", "uploads", "voice-previews");

function isInsideVoicePreviews(absPath: string) {
  const normalized = path.normalize(absPath);
  const root = path.normalize(VOICE_PREVIEWS_ROOT);
...
export async function GET(_request: Request, context: { params: Promise<{ name: string }> }) {
  ...
  const absPath = path.join(VOICE_PREVIEWS_ROOT, safeName);
  if (!isInsideVoicePreviews(absPath)) {

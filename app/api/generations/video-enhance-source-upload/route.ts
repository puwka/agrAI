...
import { MAX_TOPAZ_UPLOAD_BYTES } from "../../../../lib/transcription-limits";
...
export async function POST(request: Request) {
  ...
  if (size > MAX_TOPAZ_UPLOAD_BYTES) {
  ...
  if (supabaseUploadsEnabled()) {
    const tmpName = `topaz-${sessionUser.id}-${Date.now()}-${randomBytes(6).toString("hex")}${extLower}`;
